...
public class MsaObjectivesPreparedStatementSetter implements ItemPreparedStatementSetter<AClass> {
    @Override
        ...
        for (int i = 0; i < aClass.getAFamilies().size(); i++) {
            for (int j = 0; j < aClass.getAFamilies().get(i).getAComponentList().size(); j++) {
                if (aClass.getAFamilies().get(i).getAComponentList().get(j).getMsaApplicationNotes() != null) {
                    ...
                    preparedStatement.setString(2, String.valueOf(aClass.getAFamilies().get(i).getAComponentList().get(j).getMsaObjectives().getCombo()).replace("[","").replace("]",""));
