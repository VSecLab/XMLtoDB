...
public class AcApplicationNotesPreparedStatementSetter implements ItemPreparedStatementSetter<AClass> {
    @Override
    public void setValues(AClass item, PreparedStatement ps) throws SQLException {
        if (item.getAcApplicationNotes() != null) {
            ps.setString(1, String.valueOf(Math.random() * 32 * 41));
            ps.setString(2, String.valueOf(item.getAcApplicationNotes().getCombo())
                    .replace("[","")
                    .replace("]",""));
