...
public class AfApplicationNotesPreparedStatementSetter implements ItemPreparedStatementSetter<AClass> {
    @Override
    public void setValues(AClass item, PreparedStatement ps) throws SQLException {
        for (int i = 0; i < item.getAFamilies().size(); i++) {
            if (item.getAFamilies().get(i).getAfApplicationNotes() != null) {
                ps.setString(1, String.valueOf(Math.random() * 32 * 41));
                ps.setString(2, String.valueOf(item.getAFamilies().get(i).getAfApplicationNotes().getCombo())
                        .replace("[","")
                        .replace("]",""));
