    ...
    public void setValues(Fclass item, PreparedStatement ps) throws SQLException {
        ...
        for (int i = 0; i < item.getFfamilyList().size(); i++) {
            ...
            for (int j = 0; j < item.getFfamilyList().get(i).getFfUserNotes().getP().size(); j++) {
                ...
                ps.setString(2, item.getFfamilyList().get(i).getFfUserNotes().getP().get(j).getId_para().toUpperCase());
                ...
                ps.setString(5, item.getFfamilyList().get(i).getFfUserNotes().getP().get(j).getId_para().toUpperCase());
                ps.addBatch();
